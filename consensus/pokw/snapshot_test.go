...
// checkpoint creates a PoKW checkpoint signer section from the provided list
...
func (ap *testerAccountPool) checkpoint(header *types.Header, signers []string) {
	...
	for i, auth := range auths {
		copy(header.Extra[extraVoting+i*common.AddressLength:], auth.Bytes())
	}
}
...
func (ap *testerAccountPool) sign(header *types.Header, signer string) {
	...
	seal, _ := SealPoKWBytes(header)
	header.Sig, _ = crypto.Sign(crypto.Keccak256(seal), ap.accounts[signer])
}
...
func TestClique(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.PoKW = &params.PoKWConfig{
			Epoch:  tt.epoch,
		}
		engine := New(config.PoKW, db)
