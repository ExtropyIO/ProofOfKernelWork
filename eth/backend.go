...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.PoKW != nil {
		return pokw.New(chainConfig.PoKW,
			pokw.MinerConfig{PowMode: config.PowMode, Log: config.Log},
			db, notify, noverify)
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		s.lock.RLock()
		log.Debug("Miner is not running, init")
		price := s.gasPrice
		s.lock.RUnlock()
		...
		// if engine requires signing functionality (Clique, PoKW)
		if authEngine, ok := s.engine.(pokw.EngineWithAthorize); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			log.Info("authorizing mining wallet", "address", eb)
			authEngine.Authorize(eb, wallet.SignData)
		}
