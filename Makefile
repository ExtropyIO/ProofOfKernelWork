...
geth:
	$(GORUN) -race build/ci.go install ./cmd/geth
	@echo "Done building."
...
lint-revive-staged: GOFILES = $(shell git diff --name-only --staged  | grep ".go$$")
lint-revive-staged:
	revive -config .revive.toml $(GOFILES)

lint-revive-changed: GOFILES = $(shell { git diff --name-only; git diff --name-only --staged; } | grep ".go$$" | sort -u )
lint-revive-changed:
	revive -config .revive.toml $(GOFILES)

# need to install gotags: go get -u github.com/jstemmer/gotags
TAGS:
	find -name "*.go" | grep -v _test.go | xargs gotags > .tags
