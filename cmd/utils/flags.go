...
var (
	...
	// PoKW settings
	PoKWVerbosity = cli.IntFlag{
		Name:  "pokw.verbosity",
		Usage: "PoKW logging verbosity: 0=silent, 1=error, 2=warn, 3=info, 4=debug, 5=detail",
		Value: int(pokw.LoggerDefaultLevel),
	}
...
func setPoKW(ctx *cli.Context) {
	if ctx.GlobalIsSet(PoKWVerbosity.Name) {
		pokw.UpdateLoggingLevel(log.Lvl(ctx.GlobalInt(PoKWVerbosity.Name)))
	}
}
